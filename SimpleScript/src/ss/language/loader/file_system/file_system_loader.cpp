    ...
    file_system_loader::file_system_loader(const bool flag) {
        ...
        this->value.push_back(new ss::function("readFile", [](const size_t argc, string* argv) {
            if (is_string(argv[0])) {
                ...
                if (argc == 2) {
                    string delimiter = decode_raw(get_string(argv[1]));
                    
                    if (delimiter.empty())
                        return encode_raw(ss::read(filename));
                    
                    if (delimiter.length() == get_tablen()) {
                        size_t i = 0;
                        while (i < get_tablen() && delimiter[i] == '_')
                            ++i;
                    ...
                    int len = ss::read(buffer, filename, delimiter);
